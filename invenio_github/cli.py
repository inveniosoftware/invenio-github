...
@click.option('--sync', '-s', default=False, is_flag=True,
              help='Sync the repository prior to listing.')
@click.option('with_all', '--all', '-a', default=False, is_flag=True,
              help='List all repositories available.')
@with_appcontext
def list(user, sync, with_all):
    ...
    repos = Repository.query.filter_by(user_id=user.id)
...
@with_appcontext
def transfer(old_user, new_user, yes_i_know):
...
@click.option('--yes-i-know', is_flag=True, default=False,
              help='Suppress the confirmation prompt.')
@with_appcontext
def assign(user, repos, yes_i_know):
    ...
    Value of USER can be eitehr the email or user's ID.
...
@with_appcontext
def sync(user, hooks, async_hooks):
...
@click.option('--yes-i-know', is_flag=True, default=False,
              help='Suppress the confirmation prompt.')
@with_appcontext
def create(user, repo, yes_i_know):
...
@click.option('--yes-i-know', is_flag=True, default=False,
              help='Suppress the confirmation prompt.')
@with_appcontext
def remove(repo, user, yes_i_know):
